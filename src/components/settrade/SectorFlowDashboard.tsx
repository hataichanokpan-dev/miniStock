  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 pb-3">
        <div>
          <h3 className="text-lg font-bold text-gray-900">Sector Flow Analysis</h3>
          <p className="text-sm text-gray-500">Where is the money moving?</p>
        </div>
        {setIndex && (
          <div className="text-right">
            <p className="text-xs text-gray-500">SET Index</p>
            <p className="text-xl font-bold text-gray-900">{setIndex.toFixed(2)}</p>
            <p className={`text-sm font-bold ${getChangeColor(setChange)}`}>
              {formatPercent(setChange)}
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
          <h4 className="text-sm font-semibold text-gray-700">Money Flow (Trading Value)</h4>
          <div className="flex items-center gap-2 sm:gap-4 text-xs">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-green-500 rounded-full flex-shrink-0"></span>
              <span className="hidden sm:inline">Money In:</span> ฿{formatNumber(totalIn)}Mn
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-red-500 rounded-full flex-shrink-0"></span>
              <span className="hidden sm:inline">Money Out:</span> ฿{formatNumber(totalOut)}Mn
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            <p className="text-xs font-semibold text-green-700 mb-3">▲ Top Money In (Buying)</p>
            <div className="space-y-2">
              {moneyIn.map((sector, i) => (
                <div key={sector.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold ${i === 0 ? 'text-green-600' : 'text-green-500'}`}>
                      {i + 1}
                    </span>
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-green-600">{formatPercent(sector.chgPct)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-red-50 rounded-lg p-4 border border-red-200">
            <p className="text-xs font-semibold text-red-700 mb-3">▼ Top Money Out (Selling)</p>
            <div className="space-y-2">
              {moneyOut.map((sector, i) => (
                <div key={sector.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold ${i === 0 ? 'text-red-600' : 'text-red-500'}`}>
                      {i + 1}
                    </span>
                    <div>
                      <p className="text-sm font-medium text-gray-900">{sector.name}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-red-600">{formatPercent(sector.chgPct)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            return (
              <div key={sector.id} className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="w-16 sm:w-24 flex-shrink-0">
                  <p className="text-xs sm:text-sm font-medium text-gray-900 truncate" title={sector.name}>
                    {sector.name}
                  </p>
                  <p className="text-xs text-gray-400 hidden sm:block">{sector.id}</p>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                    ...
                    <div className="w-16 sm:w-24 text-right flex-shrink-0">
                      ...
                      <p className={`text-xs font-medium ${getChangeColor(sector.chgPct)}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-sm font-semibold text-gray-700">Market Leaders & Laggards</h4>
          <span className="text-xs text-gray-500">Vs SET: {formatPercent(marketChange)}</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {/* Leaders (Outperforming SET) */}
          <div className="bg-[#1e3a5f] rounded-lg p-4">
            <p className="text-xs font-semibold text-white/80 mb-3">⭐ Market Leaders (Beat SET)</p>
            <div className="space-y-2">
              {leaders.map((sector, i) => (
                <div key={sector.id} className="flex items-center justify-between text-white">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-yellow-400">{i + 1}</span>
                    <p className="text-sm font-medium">{sector.name}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-green-400">{formatPercent(sector.chgPct)}</p>
                    <p className="text-xs text-white/60">vs {formatPercent(marketChange)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <p className="text-xs font-semibold text-white/80 mb-3">📉 Market Laggards (Trail SET)</p>
            <div className="space-y-2">
              {laggards.map((sector, i) => (
                <div key={sector.id} className="flex items-center justify-between text-white">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-red-400">{formatPercent(sector.chgPct)}</p>
                    <p className="text-xs text-white/60">vs {formatPercent(marketChange)}</p>
                  </div>
                </div>
              ))}
      ...
      <div className="bg-gray-50 rounded-lg p-3 sm:p-4 border border-gray-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 text-center text-xs">
          <div>
            ...
            <p className="text-sm font-bold text-green-600">{moneyIn[0]?.name || '-'}</p>
            <p className="text-gray-400">฿{moneyIn[0] ? formatNumber(moneyIn[0].valMn) : 0}Mn</p>
          </div>
          <div>
            <p className="text-gray-500">Heaviest Selling</p>
            <p className="text-sm font-bold text-red-600">{moneyOut[0]?.name || '-'}</p>
            <p className="text-gray-400">฿{moneyOut[0] ? formatNumber(moneyOut[0].valMn) : 0}Mn</p>
          </div>
          <div>
            <p className="text-gray-500">Top Value Sector</p>
            <p className="text-sm font-bold text-gray-900">{topByValue[0]?.name || '-'}</p>
            <p className="text-gray-400">{topByValue[0] ? ((topByValue[0].valMn / totalMarketValue) * 100).toFixed(1) : 0}% of market</p>
          </div>
        </div>
      </div>
    </div>
